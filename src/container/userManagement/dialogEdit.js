...
export default function DialogDelete(props) {
		...
		axios(config)
			.then(function (response) {
				if (response !== []) {
					...
					setData({
						tel: response.data[0].tel_no === undefined ? '' : response.data[0].tel_no,
						bank: {
							title:
								response.data[0].bank_acc_vendor === undefined ? '' : response.data[0].bank_acc_vendor,
							value: 'FB',
						},
						bankAccount: response.data[0].bank_acc_no === undefined ? '' : response.data[0].bank_acc_no,
						firstname: response.data[0].first_name === undefined ? '' : response.data[0].first_name,
						surname: response.data[0].last_name === undefined ? '' : response.data[0].last_name,
						pin: response.data[0].pin === undefined ? '' : response.data[0].pin,
						about: {
							title: response.data[0].social_source === undefined ? '' : value,
							value: 'FB',
						},
						line: response.data[0].line_id === undefined ? '' : response.data[0].line_id,
	...
	return (
		<React.Fragment>
			{' '}
			<AlertSnackBar status={checkError} setError={setError} snackCustom={snackBar} />
			<Dialog
				maxWidth={'md'}
				open={props.open}
